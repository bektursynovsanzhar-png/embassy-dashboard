#!/usr/bin/env python3
"""
Собирает интерактивный HTML-дашборд Embassy из /data/daily.csv и /data/plans.csv.
Все исторические данные встраиваются в страницу как JSON — фильтр периода
и график по дням работают в браузере без пересборки (JS).
Пересборка (этот скрипт) нужна только когда добавляются НОВЫЕ строки данных.
"""
import csv
import json
import datetime
...
def build():
    ...
    for row in daily_rows:
        ...
        daily.append({
            ...
            "leads": num(row["leads"]),
            "pu_records": num(row["pu_records"]),
            "pu_attended": num(row["pu_attended"]),
    ...
    for row in plan_rows:
        plans.setdefault(row["month"], {})[row["project"]] = num(row["monthly_plan"])
        if row["project"] not in projects_order:
            projects_order.append(row["project"])
...
<div class="page">
  ...
  <div class="card" style="margin-bottom:16px;">
    <table>
      <thead>
        <tr><th>Проект</th><th class="num">Заявки</th><th class="num">Записей ПУ</th><th class="num">Дошли</th><th class="num">Продаж</th><th class="num">Приход</th><th class="num">% плана периода</th></tr>
      </thead>
...
function proratedPlanForDate(d, project){
  const mk = monthKey(d);
  const monthPlans = DATA.plans[mk];
  if (!monthPlans || !(project in monthPlans)) return 0;
  return monthPlans[project] / daysInMonth(d.getFullYear(), d.getMonth());
}
...
function renderRange(start, end, label){
  ...
  const agg = {};
  DATA.projects.forEach(p => agg[p] = {leads:0,pu_records:0,pu_attended:0,sales_count:0,revenue:0});
  filtered.forEach(r => {
    if (!agg[r.project]) agg[r.project] = {leads:0,pu_records:0,pu_attended:0,sales_count:0,revenue:0};
    agg[r.project].leads += r.leads;
    agg[r.project].pu_records += r.pu_records;
    agg[r.project].pu_attended += r.pu_attended;
    agg[r.project].sales_count += r.sales_count;
    agg[r.project].revenue += r.revenue + r.doplaty;
  });

  const dateList = eachDate(start, end);
  const planByProject = {};
  DATA.projects.forEach(p => planByProject[p] = 0);
  dateList.forEach(d => {
    DATA.projects.forEach(p => { planByProject[p] += proratedPlanForDate(d, p); });
  });

  const totalFact = DATA.projects.reduce((s,p)=>s+agg[p].revenue,0);
  const totalPlan = DATA.projects.reduce((s,p)=>s+planByProject[p],0);
  ...
  DATA.projects.forEach((p, idx) => {
    const plan = planByProject[p];
    const fact = agg[p].revenue;
    const pct = plan ? (fact/plan*100) : 0;
    const color = colorFor(idx);
    cardsHtml += `
    <div class="card metric">
      <div class="label">${p.toUpperCase()}</div>
      <div class="big">${fmtTg(fact)}</div>
      <div class="sub">План периода: ${fmtTg(plan)}</div>
  ...
  DATA.projects.forEach(p => {
    const a = agg[p];
    const plan = planByProject[p];
    const pct = plan ? (a.revenue/plan*100) : 0;
    rowsHtml += `<tr>
      <td>${p}</td>
...
function renderChart(dateList, filtered){
  ...
  let legend = '';
  DATA.projects.forEach((p, idx) => {
    legend += `<span><span class="dot" style="background:${colorFor(idx)}"></span>${p}</span>`;
  });
...
function applyPreset(preset, btn){
  ...
  if (preset === 'today'){
    start = new Date(today); end = new Date(today); label = 'Сегодня';
  } else if (preset === 'yesterday'){
    start = new Date(today); start.setDate(start.getDate()-1);
    end = new Date(start); label = 'Вчера';
